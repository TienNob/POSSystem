    ...
    <Container className="orderContent">
      <ListGroup className="mb-3">
        {products.map((product, index) => (
          <ListGroup.Item key={index} className="d-flex totalNav justify-content-between align-items-center flex-row-reverse">
            <div>
            ...
            <div>
              ...
              <div className="fw-bold mt-2">
                <Button variant="outline-secondary" className="me-2" onClick={() => handleDecreaseQuantity(index)}>-</Button>
                <span>{product.quantity}</span>
                <Button variant="outline-secondary" className="ms-2" onClick={() => handleIncreaseQuantity(index)}>+</Button>
              <RiDeleteBinLine className=" deleteListProduct" onClick={() => handleDeleteProduct(index)} /> 
              </div>
